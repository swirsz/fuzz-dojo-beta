 ...
 *     changes. It excludes/patches only the objects that would actively
 *     break the fuzzer build: main.o's main() (renamed via objcopy so
 *     it doesn't collide with the one $LIB_FUZZING_ENGINE supplies) and
 *     cleanup.o's _exit_cleanup() (renamed so the override below is the
 *     one that links).
 ...
 * to run.
 *
 * build.sh renames cleanup.c's real _exit_cleanup() out of the way
 * (objcopy --redefine-sym) so this definition is the one that actually
 * gets linked. None of the real one's cleanup logic (temp file removal,
 * reaping child processes, etc.) applies here -- there's no real rsync
 * session for it to clean up -- so we don't call through to it at all,
 * we just unwind back to the checkpoint set in LLVMFuzzerTestOneInput()
 * and treat this the same as a rejected/malformed input.
 *
