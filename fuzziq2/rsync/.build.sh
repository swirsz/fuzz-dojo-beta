...
# process, see the harness) to __wrap__exit_cleanup() in
# rsync_flist_fuzzer.c instead, without touching cleanup.o at all. An
# earlier version of this script tried an objcopy --redefine-sym rename
# on cleanup.o directly; that approach is more fragile (objcopy exits 0
# and silently no-ops if the symbol name doesn't match exactly what's in
# the object, so a mismatch fails silently instead of loudly) and it did
# in fact fail silently in this environment. --wrap needs no exact
# symbol-table surgery, so prefer it for this kind of interception.
RSYNC_OBJS=()
