...
# rsync.h's declarations (and struct layouts) branch on this, so the
# harness must agree with the rest of the tree or you risk silent ABI
# mismatches instead of a clean link error.
$CC $CFLAGS -DFUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION -DHAVE_CONFIG_H \
